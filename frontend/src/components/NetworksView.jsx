...
export default function NetworksView({ hostParam, isViewer }) {
  const { t } = useLang();
  const tr = t.networksView;
  ...
  const [form, setForm]             = useState({ name:'', driver:'bridge', internal:false, subnet:'', gateway:'' });
  const [creating, setCreating]     = useState(false);
  const [error, setError]           = useState('');

  const load = useCallback(async () => {
    try {
      const r = await axios.get(`/api/networks${hostParam}`);
      setNetworks(r.data);
  ...
  const handleCreate = async () => {
    ...
    try {
      await axios.post(`/api/networks${hostParam}`, form);
      setShowCreate(false);
      setForm({ name:'', driver:'bridge', internal:false, subnet:'', gateway:'' });
      load();
  ...
  const handleDelete = async (id, name) => {
    try {
      await axios.delete(`/api/networks/${id}${hostParam}`);
      setDeleting(null);
      load();
  ...
  const handleInspect = async (id) => {
    try {
      const r = await axios.get(`/api/networks/${id}/inspect${hostParam}`);
    ...
    <div style={s.wrap}>
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          <thead>
            <tr>
              {[tr.name, tr.driver, tr.scope, tr.containers, tr.subnet, tr.gateway, ''].map((h, i) => (
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} style={s.empty}>{tr.noNetworks}</td></tr>
            ) : filtered.map(net => {
              const isSystem = SYSTEM_NETWORKS.includes(net.name);
              return (
                <tr key={net.id} style={s.tr}>
                  <td style={s.td}>
                    <div style={s.nameWrap}>
                      <span style={s.netIcon}>⬡</span>
                      <span style={s.netName}>{net.name}</span>
                      {net.internal && <span style={s.badge}>internal</span>}
                      {isSystem && <span style={{...s.badge, ...s.badgeSys}}>system</span>}
                    </div>
                  </td>
                  <td style={s.td}><code style={s.code}>{net.driver}</code></td>
                  <td style={s.td}><span style={s.muted}>{net.scope}</span></td>
                  <td style={s.td}><span style={s.pill}>{net.containers}</span></td>
                  <td style={s.td}><span style={s.muted}>{net.subnet || '—'}</span></td>
                  <td style={s.td}><span style={s.muted}>{net.gateway || '—'}</span></td>
                  <td style={s.td}>
                    <div style={s.actions}>
                      <button style={s.actionBtn} onClick={() => handleInspect(net.id)}>
                        {tr.inspect}
                      </button>
                      {!isViewer && !isSystem && (
                        <button
                          style={{...s.actionBtn, ...s.actionDanger}}
                          onClick={() => setDeleting(net)}
                        >
                          {tr.delete}
                        </button>
                      )}
      ...
      {showCreate && (
        <Modal onClose={() => { setShowCreate(false); setError(''); }}>
          ...
          <div style={s.formGrid}>
            <label style={s.label}>{tr.networkName}</label>
            <input style={s.input} placeholder={tr.networkNameHint}
            ...
            <label style={s.label}>{tr.subnetLabel}</label>
            <input style={s.input} placeholder={tr.subnetHint}
              value={form.subnet} onChange={e => setForm(f => ({...f, subnet: e.target.value}))} />

            <label style={s.label}>{tr.gatewayLabel}</label>
            <input style={s.input} placeholder={tr.gatewayHint}
              value={form.gateway} onChange={e => setForm(f => ({...f, gateway: e.target.value}))} />

            <label style={s.label}>{tr.internalLabel}</label>
            <input type="checkbox" checked={form.internal}
        ...
        <Modal onClose={() => setDeleting(null)}>
          ...
          <div style={s.modalActions}>
            ...
            <button style={{...s.createBtn,...s.actionDanger}}
              onClick={() => handleDelete(deleting.id, deleting.name)}>
              {tr.delete}
            </button>
          </div>
        </Modal>
...
const s = {
  ...
  netIcon: { color:'var(--brand)', fontSize:'1em', flexShrink:0 },
  netName: { fontWeight:500, color:'var(--text-primary)' },
  badge: { fontSize:'0.68em', fontWeight:600, background:'var(--brand-glow)', color:'var(--brand-light)', border:'1px solid var(--border-focus)', borderRadius:'20px', padding:'1px 6px' },
  badgeSys: { background:'var(--bg-elevated)', color:'var(--text-muted)', border:'1px solid var(--border)' },
  code: { fontFamily:'var(--font-mono)', fontSize:'0.85em', color:'var(--text-secondary)' },
  muted: { color:'var(--text-muted)', fontSize:'0.875em' },
  pill: { background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:'20px', padding:'2px 8px', fontSize:'0.8em', color:'var(--text-secondary)' },
