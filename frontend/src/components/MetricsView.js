...
const HISTORY_SIZE = 60; // 60 puntos = ~60s a 1s de intervalo
...
function MetricCard({ container }) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const r = await axios.get(`/api/containers/${container.id}/stats`);
      const d = r.data;
      setLatest(d);
      ...
      <div style={s.metCharts}>
        ...
        <div style={s.chartBlock}>
          <div style={s.chartLabelRow}>
            ...
            <span style={{...s.chartVal, color:'var(--brand)'}}>
              {latest ? `${latest.cpu.toFixed(1)}%` : '—'}
            </span>
          </div>
          <div style={s.chartArea}>
            <Sparkline data={cpuHistory} color="var(--brand)" height={52} width={220} />
          </div>
          <div style={s.axisLabels}>
            <span>60s</span><span>0s</span>
          </div>
        </div>
        ...
        <div style={s.chartBlock}>
          <div style={s.chartLabelRow}>
            ...
            <span style={{...s.chartVal, color:'var(--warning)'}}>
              {latest ? `${fmtMem(latest.memUsage)} (${latest.memPercent.toFixed(1)}%)` : '—'}
            </span>
          </div>
          <div style={s.chartArea}>
            <Sparkline data={memHistory} color="var(--warning)" height={52} width={220} />
          </div>
          <div style={s.axisLabels}>
            <span>60s</span><span>0s</span>
          </div>
...
export default function MetricsView({ containers }) {
  const running = containers.filter(c => c.state === 'running');
  const [search, setSearch] = useState('');

  const filtered = running.filter(c =>
    ...
    <div>
      <div style={s.toolbar}>
        ...
        <div style={s.badge}>
          ...
          {filtered.length} contenedor{filtered.length !== 1 ? 'es' : ''} monitorizados · actualización cada 1s
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={s.empty}>
          ...
          <div style={s.emptyText}>No hay contenedores running</div>
...
const s = {
  ...
  axisLabels: {
    ...
    fontSize: '0.62em', color: 'var(--text-muted)', marginTop: '4px', padding: '0 2px',
  },
